...
3. Unter "Settings" → "Variables" als **Secrets** anlegen: `GEMINI_API_KEY`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_REFRESH_TOKEN`. Die Passwörter für `/search` und den Upload sind **keine Secrets dieses Workers mehr** — sie liegen zentral auf der ToolsUebersicht-Landingpage (Worker "landingpage", Secrets `PW_BELEGSCANNER_SUCHE`/`PW_BELEGSCANNER_UPLOAD`) und werden von hier aus per Server-zu-Server-Aufruf geprüft. **Wichtig:** Ohne diese Secrets dort sind Suche/Uploads gesperrt (fail-closed).
4. Unter "Settings" → "Bindings" ein **Service Binding** anlegen: Ziel-Worker `landingpage`, Variablenname `LANDINGPAGE`. (Ein normaler `fetch()` zwischen zwei Workern derselben `*.workers.dev`-Subdomain wird von Cloudflare mit Error 1042 geblockt — das Binding umgeht das.)
5. Worker deployen, die zugewiesene `*.workers.dev`-URL kopieren.
...
- Die Suche (`GET /search?q=&kategorie=&jahr=`) ist über den Scope `belegscanner-suche` geschützt (Header `X-Search-Password`). Der Upload (`POST /`) ist über den Scope `belegscanner-upload` geschützt (Header `X-Upload-Password`); `index.html` fragt das Passwort einmalig ab und merkt es sich im `localStorage` des Geräts. Beide Passwort-Prüfungen delegieren serverseitig **vor** jedem Google-Zugriff an die ToolsUebersicht-Landingpage (`verify-action-password`) und versagen „nach sicher", falls das jeweilige Secret dort fehlt oder die Landingpage nicht erreichbar ist.
