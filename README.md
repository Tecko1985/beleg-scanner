...
3. Unter "Settings" → "Variables" als **Secrets** anlegen: `GEMINI_API_KEY`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_REFRESH_TOKEN`, `SEARCH_PASSWORD`, `UPLOAD_PASSWORD` (letztere beide frei wählbare Passwörter für `/search` bzw. den Upload — vollständig eigenständig, keine Abhängigkeit von einem anderen Worker). **Wichtig:** Ohne diese beiden Secrets sind Suche/Uploads gesperrt (fail-closed).
...
- Die Suche (`GET /search?q=&kategorie=&jahr=`) ist über das Secret `SEARCH_PASSWORD` geschützt (Header `X-Search-Password`). Der Upload (`POST /`) ist über das Secret `UPLOAD_PASSWORD` geschützt (Header `X-Upload-Password`); `index.html` fragt das Passwort einmalig ab und merkt es sich im `localStorage` des Geräts. Beide Prüfungen laufen lokal im Worker (SHA-256-Digest-Vergleich, siehe `checkPassword` in `worker.js`) **vor** jedem Google-Zugriff und versagen „nach sicher", falls das jeweilige Secret fehlt.
