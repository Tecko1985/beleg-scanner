...
- Die Suche (`GET /search?q=&kategorie=&jahr=`) ist über `SEARCH_PASSWORD` geschützt (Header `X-Search-Password`). Der Upload (`POST /`) ist über `UPLOAD_PASSWORD` geschützt (Header `X-Upload-Password`); `public/index.html` fragt das Passwort einmalig ab und merkt es sich im `localStorage` des Geräts. Beide Passwort-Prüfungen laufen serverseitig **vor** jedem Google-Zugriff und versagen „nach sicher", falls das jeweilige Secret fehlt.
